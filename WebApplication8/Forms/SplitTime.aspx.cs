...
{
    public partial class SplitTime : System.Web.UI.Page
    {
            ...
            foreach (GridViewRow rows in gvTime.Rows)
                {
                    ...
                    if (cbSelect.Checked)
                    {
                        ...
                        string insertQuery = "INSERT INTO TBL_4_EXPORT_SPLITTIME (" + 
                                             "RECORDID," +
                                             "FIRSTNAME," + 
                                             "LASTNAME," + 
                                             "EMP_SEQ_NO," + 
                        ...
                        {
                            DataTier.setUploadStatus_SplitTime(recordId, BusinessTier.loggedInUser());
                            if (!DataTier.successfulCommit)
                            {
                                failedCommits++;
                                failedRecords += rows.Cells[5].Text + ",";
                                DataTier.deleteRecord(ConnectionString, "TBL_4_EXPORT_SPLITTIME", recordId);
                            }
                        }
                    }
                }
        ...
        private void loadGridviewData()
        {
            ...
            var query1 = (from a in context.tbl_4_Export_SplitTime
